...
export default function EspaceDashboard() {
  ...
  const current = etatParCode(d.etat);
  const nonLus = CONVERSATIONS.reduce((acc, c) => acc + c.nonLusCandidat, 0);

  // Map historique by état for date lookup
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <Card className="overflow-hidden border-ligne bg-blanc p-0">
            <div className="flex items-center justify-between border-b border-ligne px-6 py-4">
              <div>
                ...
                <h2 className="font-display text-lg font-bold text-encre">Les 12 étapes de votre dossier</h2>
              </div>
              <Badge className="bg-lapis/10 font-mono text-[11px] uppercase text-lapis">
                    ...
                    <li key={etat.code} className="relative flex gap-4 py-2">
                      <span
                        className={cn(
                          ...
                          isPast && "border-vert bg-vert text-blanc",
                          isCurrent && "border-lapis bg-lapis text-blanc animate-pulse-soft",
                          isFuture && "border-ligne text-ardoise"
                        )}
                      >
                        {isPast ? <CheckCircle2 className="h-4 w-4" strokeWidth={2} /> : isCurrent ? <Clock className="h-3.5 w-3.5" strokeWidth={2} /> : <Circle className="h-3 w-3" strokeWidth={1.5} />}
                      </span>
                      <div className={cn("flex-1 pt-0.5", isFuture && "opacity-55")}>
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                          ...
                          <p className={cn("text-sm font-semibold", isCurrent ? "text-lapis" : "text-encre")}>{etat.libelle}</p>
                          {date && (
                            <span className="font-mono text-[10px] text-ardoise">{formatDate(date)}</span>
                          )}
                          {isCurrent && (
                            <Badge className="bg-lapis/10 font-mono text-[9px] uppercase tracking-eyebrow text-lapis">En cours</Badge>
                          )}
                        </div>
        ...
        <RevealStagger className="space-y-4" gap={60}>
          ...
          <RevealItem>
            <Card className="border-lapis/30 bg-lapis/5 p-5">
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-lapis" strokeWidth={1.5} />
          ...
          <RevealItem>
            <Card className="border-ligne bg-blanc p-4">
              <p className="eyebrow mb-3">Raccourcis</p>
...
function ShortcutRow({ icon: Icon, label, value, tone, href }: { icon: React.ElementType; label: string; value: string; tone: "vert" | "ambre" | "ardoise"; href: string }) {
  const toneClass = { vert: "text-vert", ambre: "text-ambre", ardoise: "text-ardoise" }[tone];
  return (
    <Link href={href} className="group flex items-center gap-3 rounded-md border border-ligne px-3 py-2.5 transition-all hover:border-lapis/30 hover:bg-porcelaine">
